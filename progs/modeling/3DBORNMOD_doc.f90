...
contains
  ...
  subroutine BORNMOD_doc()
    ...
    call sep_add_doc_line("    3DBORNMOD.x - 3D/2D Born modeling - 2nd order Time/8th order Space domain FD scheme ")
    ...
    call sep_add_doc_line("    vel2 - sepfile")
    call sep_add_doc_line("            vel2.H (nz,nx,ny) = input")
    call sep_add_doc_line("            Velocity file for backward path")
    call sep_add_doc_line("            Useful for SP/PS modeling")
    call sep_add_doc_line("")
