...
contains
  ...
  subroutine LBFGS_doc()
    ...
    call sep_add_doc_line("")
    call sep_add_doc_line("    bound_type - integer: Controls when the clipping occurs with new models")
    call sep_add_doc_line("               1 (default): at each fct/gdt evaluation, even line-search")
    call sep_add_doc_line("               2          : when a new model has been found only, not during line-search")
    call sep_add_doc_line("")
