/* Antoine: 10/15/09
This has been modidied to be called from a F90 program.
Fortran passes pointers to C codes: int d in the original
C code becomes int *d in the C code called from F90 */
...
void highcut_(float *fhi,int *nphi,int *phase,int *n1,float *data, float *newdata,float *tempdata)
{
  int nodd,nb,j;
  float *b;
  ...
  for (ihi=0;ihi<nb;ihi++) {
    for (i1=2;i1<*n1;i1++)
         newdata[i1] = B(ihi,0)*data[i1]+
                       B(ihi,1)*data[i1-1]+
                       B(ihi,2)*data[i1-2]-
                       B(ihi,3)*newdata[i1-1]-
                       B(ihi,4)*newdata[i1-2];
  ...
  if (*phase == 0) {   /* highcut again in reverse */
    ...
    for (i1=2;i1<*n1;i1++) tempdata[i1] = data[*n1+1-i1];

    for (ihi=0;ihi<nb;ihi++) {
      for (i1=2;i1<*n1;i1++)
	newdata[i1] = B(ihi,0)*tempdata[i1]+ 
                      B(ihi,1)*tempdata[i1-1]+ 
                      B(ihi,2)*tempdata[i1-2]- 
                      B(ihi,3)*newdata[i1-1]- 
                      B(ihi,4)*newdata[i1-2];
...
void lowcut_(float *flo,int *nplo,int *phase,int *n1,float *data,float *newdata,float *tempdata)
{
  int nodd,nd,j;
  float *d;
  ...
  if (*phase == 0) {
	...
	newdata[i1]=D(ilo,0)*tempdata[i1]+
                    D(ilo,1)*tempdata[i1-1]+
	            D(ilo,2)*tempdata[i1-2]-
                    D(ilo,3)*newdata[i1-1]-
	            D(ilo,4)*newdata[i1-2];
