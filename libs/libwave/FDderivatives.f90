    ...
    do k=bounds%nmin3,bounds%nmax3
       do j=bounds%nmin2,bounds%nmax2
          do i=bounds%nmin1,bounds%nmax1
             ...
             val=0.
             val =  coefs%c0z*u2(i,j,k)+ &
             &      coefs%c0x*u2(i,j,k)+ &
             &      coefs%c0y*u2(i,j,k)+ &
             &      coefs%c1z*u2(i+1,j,k)+ &
             &      coefs%c1z*u2(i-1,j,k)+ &
             &      coefs%c2z*u2(i+2,j,k)+ &
             &      coefs%c2z*u2(i-2,j,k)+ &
             &      coefs%c3z*u2(i+3,j,k)+ &
             &      coefs%c3z*u2(i-3,j,k)+ &
             &      coefs%c4z*u2(i+4,j,k)+ &
             &      coefs%c4z*u2(i-4,j,k)+ &
             &      coefs%c1x*u2(i,j+1,k)+ &
             &      coefs%c1x*u2(i,j-1,k)+ &
             &      coefs%c2x*u2(i,j+2,k)+ &
             &      coefs%c2x*u2(i,j-2,k)+ &
             &      coefs%c3x*u2(i,j+3,k)+ &
             &      coefs%c3x*u2(i,j-3,k)+ &
             &      coefs%c4x*u2(i,j+4,k)+ &
             &      coefs%c4x*u2(i,j-4,k)+ &
             &      coefs%c1y*u2(i,j,k+1)+ &
             &      coefs%c1y*u2(i,j,k-1)+ &
             &      coefs%c2y*u2(i,j,k+2)+ &
             &      coefs%c2y*u2(i,j,k-2)+ &
             &      coefs%c3y*u2(i,j,k+3)+ &
             &      coefs%c3y*u2(i,j,k-3)+ &
             &      coefs%c4y*u2(i,j,k+4)+ &
             &      coefs%c4y*u2(i,j,k-4)

             u3(i,j,k)=mod%vel(i,j,k)**2*val
          end do
       end do
    end do
    !$OMP END PARALLEL DO
  ...
  subroutine FD_2nd_2D_derivatives_scalar_adjoint_grid(genpar,bounds,u2,u3,mod)
    ...
    do j=bounds%nmin2,bounds%nmax2
       do i=bounds%nmin1,bounds%nmax1
          ...
          &      coefs%c1z*(mod%vel(i+1,j,1)**2*u2(i+1,j,1)+mod%vel(i-1,j,1)**2*u2(i-1,j,1))+ &
          &      coefs%c2z*(mod%vel(i+2,j,1)**2*u2(i+2,j,1)+mod%vel(i-2,j,1)**2*u2(i-2,j,1))+ &
          &      coefs%c3z*(mod%vel(i+3,j,1)**2*u2(i+3,j,1)+mod%vel(i-3,j,1)**2*u2(i-3,j,1))+ &
          &      coefs%c4z*(mod%vel(i+4,j,1)**2*u2(i+4,j,1)+mod%vel(i-4,j,1)**2*u2(i-4,j,1))
          tmpxx= coefs%c0x*(mod%vel(i  ,j,1)**2*u2(i,j  ,1)) +&
          &      coefs%c1x*(mod%vel(i,j+1,1)**2*u2(i,j+1,1)+mod%vel(i,j-1,1)**2*u2(i,j-1,1))+ &
          &      coefs%c2x*(mod%vel(i,j+2,1)**2*u2(i,j+2,1)+mod%vel(i,j-2,1)**2*u2(i,j-2,1))+ &
          &      coefs%c3x*(mod%vel(i,j+3,1)**2*u2(i,j+3,1)+mod%vel(i,j-3,1)**2*u2(i,j-3,1))+ &
          &      coefs%c4x*(mod%vel(i,j+4,1)**2*u2(i,j+4,1)+mod%vel(i,j-4,1)**2*u2(i,j-4,1))
