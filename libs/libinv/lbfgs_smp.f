...
C     ****************
C
      SUBROUTINE LBFGSS(N,M,X,F,G,DIAGCO,DIAG,IPRINT,EPS,XTOL,W,IFLAG,
     *                 MYINFO)
C
...
C             are possible:
C 
C              IFLAG=-1  The line search routine MCSRCHS failed. The
C                        parameter INFO provides more detailed information
C                        (see also the documentation of MCSRCHS):
C
...
C                       EXTERNAL LB2S
C
C    LB2S is a BLOCK DATA that defines the default values of several
...
C        controls the accuracy of the line search routine MCSRCHS. If the
...
C 
C    Other routines called directly:  DAXPY, DDOT, LB1S, MCSRCHS
C 
...
C     BY USING THE LINE SEARCH ROUTINE MCSRCHS
C     ----------------------------------------------------
 165  NFEV=0
      STP=ONE
...
C
 200  FORMAT(/' IFLAG= -1 ',/' LINE SEARCH FAILED. SEE'
     .          ' DOCUMENTATION OF ROUTINE MCSRCHS',/' ERROR RETURN'
...
C
C
      SUBROUTINE LB1S(IPRINT,ITER,NFUN,
     *                     GNORM,N,M,X,F,G,STP,FINISH)
C
C     -------------------------------------------------------------
...
C
      ...
      COMMON /LB3S/MP,LP,GTOL,STPMIN,STPMAX
      DATA MP,LP,GTOL,STPMIN,STPMAX/0,0,0.9D+00,1.0D-20,1.0D+20/
      END
C
C
C   ----------------------------------------------------------
...
C
C     **************************
C     LINE SEARCH ROUTINE MCSRCHS
C     **************************
C
      SUBROUTINE MCSRCHS(N,X,F,G,S,STP,FTOL,XTOL,MAXFEV,INFO,NFEV,WA)
...
C
C     THE PURPOSE OF MCSRCHS IS TO FIND A STEP WHICH SATISFIES
...
C
C        SUBROUTINE MCSRCHS(N,X,F,G,S,STP,FTOL,XTOL, MAXFEV,INFO,NFEV,WA)
C     WHERE
C
...
c         write(0,*) 'ftest1',ftest1,'finit',finit,'stp',stp,'dgtest',
c     *        dgtest
C
...
C
            CALL MCSTEPS(STX,FXM,DGXM,STY,FYM,DGYM,STP,FM,DGM,
...
C
         ...
         ELSE
C
C           CALL MCSTEPS TO UPDATE THE INTERVAL OF UNCERTAINTY
C           AND TO COMPUTE THE NEW STEP.
C
            CALL MCSTEPS(STX,FX,DGX,STY,FY,DGY,STP,F,DG,
     *                 BRACKT,STMIN,STMAX,INFOC)
            END IF
C
...
C
         GO TO 30
C
C     LAST LINE OF SUBROUTINE MCSRCHS.
C
      END
      SUBROUTINE MCSTEPS(STX,FX,DX,STY,FY,DY,STP,FP,DP,BRACKT,
     *                 STPMIN,STPMAX,INFO)
      INTEGER INFO
      DOUBLE PRECISION STX,FX,DX,STY,FY,DY,STP,FP,DP,STPMIN,STPMAX
      LOGICAL BRACKT,BOUND
C
C     SUBROUTINE MCSTEPS
C
C     THE PURPOSE OF MCSTEPS IS TO COMPUTE A SAFEGUARDED STEP FOR
...
C     THE SUBROUTINE STATEMENT IS
C
C       SUBROUTINE MCSTEPS(STX,FX,DX,STY,FY,DY,STP,FP,DP,BRACKT,
...
C
      ...
      RETURN
C
C     LAST LINE OF SUBROUTINE MCSTEPS.
C
      END
