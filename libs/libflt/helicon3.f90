...
contains
  subroutine helicon3_mod_init ( aa_in )
    type( filter)    :: aa_in 
    aa = aa_in
  end subroutine helicon3_mod_init
  function helicon3_mod_lop ( adj, add, xx, yy) result(stat)
    integer            :: stat 
    logical,intent(in) :: adj,add 
    real,dimension(:)  :: xx,yy 
    call adjnull (adj,add,xx,yy )
    call helicon3_mod_lop2(adj,add,xx,yy )
    stat=0
  end function helicon3_mod_lop
  subroutine helicon3_mod_lop2(adj,add,xx,yy)
    logical,intent(in) :: adj,add 
    real, dimension (:)  :: xx 
    real, dimension (:)  :: yy 
    integer iy, ix, ia
