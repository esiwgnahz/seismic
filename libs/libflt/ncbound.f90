...
module ncbound  
  ...
  implicit none
contains 
  subroutine ncboundn ( nold, nd, na, aa)
    ...
    call nchelicon_init( aa)                     ! give aa pointer to helicon.lop
    call regridn( nold, nb, aa)
    aa%flt = 1.                ! put all 1's in filter
    stat = nchelicon_lop( .false., .false., xx, yy)        ! apply filter
